...
while (have_posts()) : the_post();
    ...
    if ($transaction_id != '') {
        ...
        if ($transaction_id != 'free') {
            ...
            if (!empty($transaction_id) && !empty($st)) {
                ...
                $data = $wpdb->get_results($sql_select, ARRAY_A);
                print_r($data);
                if (empty($data)) {
    ...
    <div <?php post_class('container'); ?> id="post-<?php the_ID(); ?>">
            ...
            <div class="<?php echo esc_attr($pageOptions['class']); ?>" id="content-col">
                ...
                if (!empty($event_date)) {
                    ...
                    if (!empty($eventStartDate) && !empty($eventEndDate) && $eventStartDate !== $eventEndDate) {
                        $date = '<strong>' . $s_day_name . '</strong> | ' . $eventStartDate . esc_html__(' to ', 'framework') . ' <strong>' . $e_day_name . '</strong> | ' . $eventEndDate;
                    } else {
                    ...
                    if (!empty($eventStartDate) && !empty($eventEndDate) && $eventStartDate !== $eventEndDate) {
                        $date = '<strong>' . date_i18n('l', $eventStartDate) . '</strong> | ' . date_i18n(get_option('date_format'), $eventStartDate) . esc_html__(' to ', 'framework') . ' <strong>' . date_i18n('l', $eventEndDate) . '</strong> | ' . date_i18n(get_option('date_format'), $eventEndDate);
                    } else {
                        if (!empty($eventStartDate)) {
                ...
                if (get_query_var('event_date')) {
                    $eventDetailsData = array(
                        $event_dt_out[1], $event_dt_out[0], get_post_meta(get_the_ID(), 'imic_event_address', true),
                    ...
                    foreach ($events_all as $key => $value) {
                        if ($value == get_the_ID()) {
                            ...
                            if ($days_extra > 0) {
                                ...
                                $date_opt =  $st_dt_unix . ' ' . esc_html__('to', 'framework') . ' ' . $en_dt_unix;
                            } else {
                    ...
                    $eventDetailsData = array(
                        $date_opt, $event_dt_out[0], get_post_meta(get_the_ID(), 'imic_event_address', true),
                        get_post_meta(get_the_ID(), 'imic_event_contact', true)
                    );
                }
                ...
                if ($registration_status == 1 && function_exists('imic_get_currency_symbol')) {
                    ...
                    $eventPersonDetails = array(esc_html__('Attendees', 'framework'), esc_html__('Staff members', 'framework'), esc_html__('Registration', 'framework'));
                    if (function_exists('causes_option_page') && ($registration_status == 1)) {
                    ...
                    $eventPersonDetails = array(esc_html__('Attendees', 'framework'), esc_html__('Staff members', 'framework'));
                    $eventPersonData = array(get_post_meta(get_the_ID(), 'imic_event_attendees', true), get_post_meta(get_the_ID(), 'imic_event_staff_members', true));
                    $eventPersonValues = array_filter($eventPersonData, 'strlen');
                }
                ...
                <header class="single-post-header clearfix">
                    <nav class="btn-toolbar float-end">
                        <a href="javascript:" onclick="window.print();" class="btn btn-default" data-placement="bottom" data-bs-toggle="tooltip" data-original-title="<?php _e('Print', 'framework'); ?>" rel="tooltip"><i class="fa fa-print"></i></a>
                        <a href="mailto:<?php echo esc_attr($event_email); ?>" class="btn btn-default" data-placement="bottom" data-bs-toggle="tooltip" data-original-title="<?php _e('Contact us', 'framework'); ?>" rel="tooltip"><i class="fa fa-envelope"></i></a>
                        <?php $imic_event_address = get_post_meta(get_the_ID(), 'imic_event_address', true);
                        if (!empty($imic_event_address)) {
                            echo '<a href="http://maps.google.com/?q=' . $imic_event_address . '" class="btn btn-default" data-placement="bottom" data-bs-toggle="tooltip" data-original-title="' . esc_html__('Event Address', 'framework') . '" rel="tooltip" target="_blank"><i class="fa fa-map-marker"></i></a>';
                        }
                        ...
                        if ($custom_registration_url != '') {
                            echo '<a href="' . $custom_registration_url . '" class="btn btn-primary donate-paypal custom-event-registration-url" ' . $crut . '>' . esc_html__('Register', 'framework') . '</a>';
                        } else {
                            if (function_exists('causes_option_page') && ($registration_status == 1)) {
                                if (is_user_logged_in()) {
                                    echo '<a href="#" id="donate-popup" class="btn btn-primary donate-paypal" data-bs-toggle="modal" data-bs-target="#PaymentModal">' . esc_html__('Register', 'framework') . '</a>';
                                } else {
                                    echo '<a href="#" id="login-register" class="btn btn-primary donate-paypal" data-bs-toggle="modal" data-bs-target="#PaymentModal">' . esc_html__('Register', 'framework') . '</a>';
                                }
                            }
                        }
                        ?>
                ...
                <article class="post-content">
                    <div class="event-description">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
								...
								if (isset($options['switch_sharing']) && $options['switch_sharing'] == 1) {
									if($options['share_post_types']['3'] == '1'){
										imic_share_buttons();
									}
								}
                        ...
                        <div class="dropdown nc-save-event">
                            <button id="dLabel" class="btn btn-default" type="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                <?php _e('Save Event to Calendar', 'framework'); ?>
                                <span class="caret"></span>
                            </button>
                            <ul class="dropdown-menu" aria-labelledby="dLabel">
                                <li><a target="_blank" href="<?php echo esc_url(add_query_arg(array('action' => 'icalendar', 'edate' => $sb, 'id' => get_the_ID(), 'key' => 'imic_save_event'))); ?>" title="<?php _e('Save to iCalendar', 'framework'); ?>"><i class="fa fa-calendar-plus-o"></i> <?php _e('Save to iCalendar', 'framework'); ?></a></li>
                                <li><a target="_blank" href="<?php echo esc_url(add_query_arg(array('action' => 'gcalendar', 'edate' => $sb, 'id' => get_the_ID(), 'key' => 'imic_save_event'))); ?>" title="<?php _e('Save to Google Calendar', 'framework'); ?>"><i class="fa fa-google"></i> <?php _e('Save to Google Calendar', 'framework'); ?></a></li>
                                <li><a target="_blank" href="<?php echo esc_url(add_query_arg(array('action' => 'outlook', 'edate' => $sb, 'id' => get_the_ID(), 'key' => 'imic_save_event'))); ?>" title="<?php _e('Save to Outlook', 'framework'); ?>"><i class="fa fa-envelope-o"></i> <?php _e('Save to Outlook', 'framework'); ?></a></li>
                                <li><a target="_blank" href="<?php echo esc_url(add_query_arg(array('action' => 'outlooklive', 'edate' => $sb, 'id' => get_the_ID(), 'key' => 'imic_save_event'))); ?>" title="<?php _e('Save to Outlook Online', 'framework'); ?>"><i class="fa fa-cloud-download"></i> <?php _e('Save to Outlook Online', 'framework'); ?></a></li>
                                <li><a target="_blank" href="<?php echo esc_url(add_query_arg(array('action' => 'yahoo', 'edate' => $sb, 'id' => get_the_ID(), 'key' => 'imic_save_event'))); ?>" title="<?php _e('Save to Yahoo! Calendar', 'framework'); ?>"><i class="fa fa-yahoo"></i> <?php _e('Save to Yahoo! Calendar', 'framework'); ?></a></li>
                            </ul>
                        </div>
                    </div>
                </article>
        ...
        <div class="modal fade" id="PaymentModal" tabindex="-1" data-bs-backdrop="static" role="dialog" aria-labelledby="PaymentModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        } elseif ($guest_registration == 1) {
                            echo do_shortcode('[imic_events amount="' . $event_registration_fee . '" event_id="' . get_the_ID() . '" description="' . get_the_title() . '" return="' . $custom_event_url . '"]');
                        } else { ?>
                            <div class="tabs">
                                <ul class="nav nav-tabs">
                                    <li class="active"> <a data-bs-toggle="tab" href="#login-user-form"> <?php _e('Login', 'framework'); ?> </a> </li>
                                    <li> <a data-bs-toggle="tab" href="#register-user-form"> <?php _e('Register', 'framework'); ?> </a> </li>
                                </ul>
