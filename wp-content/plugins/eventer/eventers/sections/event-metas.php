<?php
$single_date_format = eventer_get_settings('eventer_date_format_single');
if($single_date_format == ''){
	$single_date_format = get_option('date_format');
}
wp_localize_script('eventer-single-scripts', 'single_meta', array('future_date_cal' => esc_attr(date_i18n($single_date_format, $event_cdate))));
?>
<div class="eventer eventer-event-single eventer-single-event-details">
  ...
  <ul class="eventer-single-event-info">
    ...
    $virtual_event = get_post_meta(get_the_ID(), 'eventer_event_virtual', true);
    if (!is_wp_error($eventer_venue) && !empty($eventer_venue)) {
      foreach ($eventer_venue as $venue) {
        ...
        echo '<li><span class="eventer-event-venue"><i class="eventer-icon-location-pin"></i> ' . eventer_get_event_venue( $venue_name, get_the_ID() ) . '</span></li>';
      }
    } elseif ($specific_address != '') {
      echo '<li><span class="eventer-event-venue"><i class="eventer-icon-location-pin"></i> ' . eventer_get_event_venue( $specific_address, get_the_ID() ) . '</span></li>';
    }
    if ($days_diff <= 0) {
      echo '<li><span class="eventer-event-date"><i class="eventer-icon-calendar"></i> ' . esc_attr(date_i18n($single_date_format, $event_cdate)) . '</span></li>';
      ...
      echo '<li><span class="eventer-event-date"><i class="eventer-icon-calendar"></i> ' . esc_attr(date_i18n($single_date_format, $event_cdate)) . ' - ' . esc_attr(date_i18n($single_date_format, strtotime($set_end_date))) . '</span></li>';
    }
    ?>
    ...
    if (!is_wp_error($event_organizer) && !empty($event_organizer)) {
      ...
      echo '<li><span class="eventer-event-time"><i class="eventer-icon-user"></i> ' . $event_organizer[0]->name . '</span></li>';
      if ($organizer_phone) {
    ...
    if (empty($default) || !empty($links)) {
      ...
      <ul class="eventer-actions">
        ...
        <?php
            ...
            ?>
        <?php if ($virtual_event != 'on' && ((empty($default) && $elocation) || ($elocation && in_array('direction', $links)))) {
