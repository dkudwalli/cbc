...
$ticket_enabled_date = $bookingClsoed = '';
  ...
  <div class="eventer-ticket-details-wrap">
    ...
    if (!empty($show_tickets_info)) {
      ...
      foreach ($show_tickets_info as $ticket) {
        ...
        $remaining_tickets = ($ticket_number <= 0) ? '<i class="eventer-ticket-remaining eventer-ticket-full">' . esc_html__('All Booked', 'eventer') . '</i>' : '<i class="eventer-ticket-remaining" data-tickets="' . $ticket_number . '">' . $ticket_number . ' ' . esc_html__('remaining', 'eventer') . '</i>';
        if ($ticket_number > 0) {
          $remaining_for_reg = 1;
        }
      ...
      echo '</ul><div style="padding-top: 10px;">' . $ticket_enabled_date . '</div></div>';
    }
