...
if (!function_exists('load_custom_wp_admin_scripts')) {
  ...
  function load_custom_wp_admin_scripts($hook)
  {
    ...
    wp_enqueue_style('eventer_datetimepicker', EVENTER__PLUGIN_URL . 'css/jquery.simple-dtpicker.css', array(), $plugin_version, 'all');
    wp_enqueue_script('eventer_datetimepicker', EVENTER__PLUGIN_URL . 'js/jquery.simple-dtpicker.js', array('jquery-ui-core'), $plugin_version, true);
    wp_enqueue_script('eventer_ui_plugins', EVENTER__PLUGIN_URL . 'js/jquery-ui.multidatespicker.js', array('jquery-ui-core'), $plugin_version, true);
    ...
    if (isset($_REQUEST['page']) == 'eventer_settings_options&tab=bookings') {
		wp_enqueue_script('eventer_admin_standard_bookings', EVENTER__PLUGIN_URL . 'js/standard_bookings.js', array('jquery'), $plugin_version, true);
    }
	// Add JS for Eventer design settings page
    if (isset($_REQUEST['page']) == 'eventer_settings_options&tab=design') {
		wp_enqueue_style('select2-css', 'https://cdnjs.cloudflare.com/ajax/libs/select2/4.1.0-beta.1/css/select2.min.css');
		wp_enqueue_script('select2-js', 'https://cdnjs.cloudflare.com/ajax/libs/select2/4.1.0-beta.1/js/select2.min.js', array('jquery'), '4.1.0-beta.1', true);
    	wp_enqueue_script('eventer-admin-select2-js', EVENTER__PLUGIN_URL . 'js/admin-select2.js', array('jquery', 'select2-js'), null, true);
  	}
  }
...
if (!function_exists('eventer_add_form_fields_venue')) {
  ...
  function eventer_add_form_fields_venue($term, $taxonomy)
  {
	...
	*/
      function eventer_find_first_recurring_events($ID)
      {
        ...
        if ($eventer_frequency_count > 0) {
          $eventer_frequency_val = ($eventer_frequency == 365) ? 'year' : (($eventer_frequency == 30) ? 'month' : 'days');
          $eventer_frequency = ($eventer_frequency == 365) ? 1 : (($eventer_frequency == 30) ? 1 : $eventer_frequency);
      ...
      function eventer_find_second_recurring_events($ID)
      {
        ...
        if ($event_weekly === 'on') {
          ...
          for ($i = 0; $i < $eventer_frequency_count; $i++) {
            ...
            $next_month = date('F', $event_start_date_next);
	...
	*/
    ...
    //add_action('init', 'newPrimary');
    function newPrimary()
    {
      global $wpdb;
      $table_name = $wpdb->prefix . "eventer_registrant";
      $wpdb->query("ALTER TABLE {$table_name} AUTO_INCREMENT = 20000;");
    }
	...
	*/
      add_filter('manage_edit-eventer_sortable_columns', 'eventer_set_eventer_column_label');
	...
	*/
      ...
      if (!empty($wpdb->last_error)) {
        ...
        if ($woocommerce_events != 'on') {
          ...
          foreach ($export_query as $Result) {
            ...
            $checkin_info = (isset($user_system['checkin'])) ? $user_system['checkin'] : '';
            $checkin_info = ($checkin_info == '1') ? 'Yes-' . $user_system['checkin_date'] : $checkin_info;
            $service = '';
            if ($services) {
              ...
              foreach ($registrants as $key => $value) {
                ...
                if (!empty($value)) {
                  ...
                  foreach ($value as $regs) {
                    ...
                    $set_arr = array('status' => $Result['status'], 'title' => get_the_title($Result['eventer']), 'id' => $Result['id'], 'event_date' => $Result['eventer_date'], 'event_slot' => $time_slot, 'registration_date' => $Result['ctime'], 'username' => $Result['username'], 'email' => $Result['email'], 'amount' => $Result['amount'], 'count' => $tickets_booked[$key], 'QR_ID' => $qr_code, 'check_in' => $checkin_info, 'services' => $service, 'registrant_name' => $name, 'registrant_email' => $email, 'registrant_ticket' => $key);
                    if (!empty($user_data)) {
                    ...
                    foreach ($regs as $field_key => $value ) {
                      if( $field_key == 'name' || $field_key == 'email' || $field_key == 'QR_ID' ) continue;
                      $set_arr[$field_key] = $value;
                    }
                    $leadArray[] = $set_arr;
                  }
                }
          ...
          foreach ($export_query as $Result) {
            ...
            foreach ($order->get_items() as $item_key => $item_values) :
              ...
              $data_set['Booking Date'] = date_i18n(get_option('date_format') . ' ' . get_option('time_format'), strtotime($Result['ctime']));
              $data_set['Mode'] = $order->get_payment_method_title();
              $data_set['Name'] = $order->get_billing_first_name() . ' ' . $order->get_billing_last_name();
              $data_set['Email'] = $order->get_billing_email();
      ...
      if ($woocommerce_events == 'on') {
        ...
        if (!empty($tickets)) {
          if ($tickets->ticket_status != 10) {
            updateTicket($ticket_id, ['ticket_status' => 10], ['%d']);
          ...
          $ticket_info = '<div class="form-style-2">
                        <div class="form-style-2-heading">'.esc_html__('Ticket Information','eventer').'</div>
                        <label><span>Name </span>' . $tickets->user_name . '</label>
                    </div>';
        } else {
    ...
    function eventer_create_page_for_shortcode_preview()
    {
      ...
      $shortcode_preview = array(
        ...
        'post_content'  => esc_html__('This page show preview of shortcode that you have generated from the settings page.', 'eventer'),
        'post_status'   => 'publish',
        'post_author'   => 1,
        'post_type' => 'page'
      );
    ...
    function eventerGetFirstKey(array $array)
    {
      foreach ($array as $key => $value) {
        return $key;
      }
    }
// Ticket Meta insertion into DB
	function addDefaultMeta($data)
	{
		global $wpdb;
		$table_name = $wpdb->prefix . "eventer_default_ticket_meta";
		$wpdb->query(
			$wpdb->prepare(
				"INSERT INTO $table_name
		  ( reg_id, meta, slot, value)
		  VALUES ( %d, %s, %s, %s )",
				array($data['reg_id'], $data['meta'], $data['slot'], $data['value'])
			)
		);
		return $wpdb->insert_id;
	}
	function getDefaultMeta($regId, $metaKey)
	{
		global $wpdb;
	...
	function updateDefaultMeta($regId, $metaKey, $metaValue)
	{
		global $wpdb;
		$tableName = $wpdb->prefix . "eventer_default_ticket_meta";
		if (!empty(getTicketMeta($regId, $metaKey))) {
			$wpdb->update($tableName, ['value' => $metaValue], array('reg_id' => $regId, 'meta' => $metaKey), array('%s'), array('%d', '%s'));
		} else {
			addTicketMeta(['reg_id' => $regId, 'meta' => $metaKey, 'value' => $metaValue]);
		}
	}

// List Google Fonts
function eventer_get_google_fonts() {
    $transient_key = 'eventer_google_fonts';
    $cached_fonts = get_transient($transient_key);

    if ($cached_fonts === false) {
        $api_key = 'AIzaSyDMkY9wxlDJgIDUaQAHHIYDvvECGPC0pPY';
        ...
        if (isset($data['items'])) {
            $fonts = wp_list_pluck($data['items'], 'family');
            set_transient($transient_key, $fonts, 15 * DAY_IN_SECONDS); // Cache for 12 hours
        } else {
            return [];
        }
    } else {
