...
function eventer_show_title($title = '', $id = '')
{
  ...
  //$title .= ($passed_badge != '' && eventer_get_settings('eventer_show_badges') == 'on') ? $passed_badge : '';
...
function eventer_show_title_listing($title = '', $id = '', $all_data = array())
{
  ...
  $event_edate = (isset($all_data['event_edate'])) ? $all_data['event_edate'] : date_i18n('U');
  $event_edate = (!empty($event_edate)) ?   $event_edate :   $event_cdate;
  $booked_tickets_create = (!empty($all_data) && isset($all_data['booked_tickets'])) ? $all_data['booked_tickets'] : '';
  $booked_tickets_status = (!empty($booked_tickets_create) && isset($booked_tickets_create[0]['label']) && $booked_tickets_create[0]['label'] != '') ? '<span class="eventer-status-badge eventer-custom-label-badge">' . $booked_tickets_create[0]['label'] . '</span>' : '';
...
function eventer_formatting_email_content($content = '', $registrant_details = array(), $email_content = '')
{
  ...
  $vars = array(
    ...
    '{event_additional_info}' => wpautop(get_post_meta($eventer_id, 'eventer_event_email_additional_info', true)),
    '{tkt}'           => $tickets,
...
function eventer_formatting_woo_email_content($content = '', $order_id = '', $email_content = '')
{
  ...
  $vars = array(
    ...
    '{tx_id}'         => esc_attr($order->get_transaction_id()),
    '{pmt_st}'         => esc_attr($order->get_status()),
    '{reg_email}'     => $registrant_email,
    '{amt_pd}'         => esc_attr($order->get_total()),
    '{evt_date}'         => esc_attr($eventer_date_formatted),
    ...
    '{event_additional_info}' => get_post_meta($event_id, 'eventer_event_email_additional_info', true)
  );
      ...
      $eventer_location = ($location_address != '') ? $location_address : $venue->name;
        ...
        if (!empty($valvalue) && $valindex != 'main') {
          foreach ($valvalue as $val) {
            ...
            $individual[] = array('data-ticket' => $new_valindex, 'data-elocation' => $eventer_location, 'data-datetime' => $time . ' ' . $date, 'data-eventid' => $event_id, 'data-eventname' => get_the_title($event_id), 'data-email' => $val['email'], 'data-name' => $val['name'], 'data-qrcode' => $reg->id, 'data-img' => '');
          }
        }
      }
    }
...
function eventer_ticket_raw_html($ticket_show = '', $params = array())
{
    ...
    if ( isset( $params['default'] ) && isset( $params['default']['data-eid'] ) ) {
        $event_id = $params['default']['data-eid'];
    } else {
        $event_id = get_the_ID();
        $params['default']['data-eid'] = $event_id;
    }
  ...
  <div id="eventer-ticket-printable" class="eventer-ticket-final-tickets eventer-ticket-image_create" data-eid="'. esc_attr($event_id) .'" data-nonce="' . esc_attr($params['data-nonce']) . '"' . $default . '>
      <div class="eventer-ticket-printable">
        ...
        <div class="eventer-ticket-printable-bottom">
          ...
          <p class="registrant-ticket">' . esc_attr(get_the_title($params['default']['data-eid'])) . '</p>
          <div class="eventer-spacer-10"></div>
      ...
      if ($event_notes) {
        $ticket_show .= '<div class="eventer-spacer-10"></div><div class="eventer-row"><div class="eventer-col10 eventer-pt-instructions">
                          <label>' . esc_html__('Instructions', 'eventer') . '</label>
                          <p>' . wp_kses( $event_notes, 'post' ) . '</p>
                        </div>
              </div>';
      }
      $ticket_show .= '<div class="eventer-spacer-10"></div>
          <div class="eventer-row eventer-ticket-c-info">
                <div class="eventer-col10 eventer-pt-cominfo">';
  if ($company_logo && $company_image ) {
    $ticket_show .= '<p class="eventer-ticket-c-logo">' . $company_image . '</p>';
  }
  if ($company_name) {
    $ticket_show .= '<p class="eventer-ticket-c-address">'.esc_attr($company_name).'</p>';
  }
  if ($company_address) {
    $ticket_show .=  '<p class="eventer-ticket-c-address">'.$company_address.'</p>';
  }
...
function eventer_dequeue_scripts()
{
  ...
  if (is_singular('eventer')) {
    ...
    //wp_dequeue_script('countdown'); //Bridge theme
  }
  wp_dequeue_script('countdown'); //Bridge theme
}
