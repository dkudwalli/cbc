...
class eventer_stage2_generate_shortcode {
  ...
  function eventer_filters_view( $link = '', $status = '', $series = '' ) {
    ...
    // normalize status to expected values to ensure downstream parameters exist
    $allowed_statuses = array( 'monthly', 'yearly', 'weekly', 'daily' );
    $status = ( in_array( $status, $allowed_statuses, true ) ) ? $status : $status;
    ...
    $status = ( in_array( $status, $allowed_statuses, true ) ) ? $status : 'monthly';
    ...
    $parameters[ 'span' ] = '';
    // initialize defaults to avoid undefined index notices in templates
    $parameters[ 'view' ] = ( isset( $parameters[ 'view' ] ) ) ? $parameters[ 'view' ] : '';
    $parameters[ 'steps' ] = ( isset( $parameters[ 'steps' ] ) ) ? $parameters[ 'steps' ] : 0;
    $parameters[ 'type' ] = ( isset( $parameters[ 'type' ] ) ) ? $parameters[ 'type' ] : '';
    $parameters[ 'view_format' ] = ( isset( $parameters[ 'view_format' ] ) ) ? $parameters[ 'view_format' ] : '';
    if ( $status == 'monthly' ) {
        ...
        if ( !empty( $parameters ) ) {
          ...
          if ( $badge_switch == 'on' ) {
            if ( date_i18n( 'U' ) < strtotime( $key ) ) {
              ...
              $parameters[ 'status' ] = '<span class="eventer-status-badge eventer-status-upcoming">' . esc_attr__( ' Ongoing', 'eventer' ) . '</span>';
            } else {
