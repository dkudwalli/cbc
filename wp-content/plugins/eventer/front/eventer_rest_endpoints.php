  ...
  if ($woocommerce_events == 'on') {
    ...
    if (!empty($tickets)) {
      if ($tickets->ticket_status != 10) {
        ...
        //$message = "This ticket is already checked in";
      }
      ...
      $eventers = array('ID' => $code, 'Title' => "", 'Date' => $tickets->ticket_date, 'name' => $tickets->user_name, 'email' => $email, "status" => $status, "amount" => $amount);
    } else {
      $eventers = array('ID' => "No such ticket", 'Title' => "", 'Date' => "", 'name' => "", 'email' => "", "status" => "", "amount" => "");
...
function eventer_reset_password_endpoint($request = null)
{
  ...
  if ($exists) {
    ...
    if (empty($already_verified) || $remaining_time > 300 || $resend == '1') {
      ...
      $message .= '</div>';
      send_eventer_custom_email($username, esc_html__('Reset password verficiation code', 'eventer'), $message, $headers);
      $already_verified_timer = get_user_meta($exists, 'eventer_verification_code_timer', true);
      $remaining_time = date_i18n('U') - $already_verified_timer;
      $response['seconds'] = ($remaining_time > 300) ? 300 : 300 - $remaining_time;
    }
...
function eventer_email_endpoint($request = null)
{
  ...
  if ($user_info && $email_content_switch != '0') {
    ...
    $email_status = send_eventer_custom_email($user_info->user_email, esc_html__('New event added', 'eventer'), $new_message, $headers);
  }
