      ...
      foreach ($eventer as $event_data) {
        ...
        if (!empty($eventer_data)) {
          ...
          if ($eventer_data['elocation'] != '') {
            $countdown_output .= '<span class="eventer-event-venue"><i class="eventer-icon-location-pin"></i> ' . eventer_get_event_venue( $eventer_data['elocation'], $value ) . '</span>';
          }
  ...
  if (!function_exists('eventer_calendar_shortcode')) {
	...
	*/
    function eventer_calendar_shortcode($atts)
    {
      ...
      $calendar_rtl = (eventer_get_settings('eventer_calendar_rtl') == '1') ? 'yes' : 'no';
      $calendar_today = (eventer_get_settings('eventer_calendar_start_today') == '1') ? 'yes' : 'no';
      ...
      switch ($atts['type']) {
        case 'textarea':
          ...
          if ($atts['textarea_type'] != 'wp') {
            $field = '<label>' . esc_attr($atts['label']) . $required . '</label> <textarea data-meta="' . esc_attr($atts['meta_key']) . '" ' . $required_data . ' rows="' . esc_attr($atts['text_row']) . '" ' . $name . ' ' . $placeholder . ' ' . $class . ' ' . $id . '></textarea>';
          } else {
            ob_start();
        ...
        case 'radio':
          ...
          break;
        case 'number':
          $field = '<label>' . esc_attr($atts['label']) . $required . '</label> <input data-meta="' . esc_attr($atts['meta_key']) . '" type="number" ' . $required_data . ' value="" ' . $name . ' ' . $placeholder . ' ' . $class . ' ' . $id . '>';
          break;
        case 'div':
        ...
        case 'email':
          ...
          $field = '<label>' . esc_attr($atts['label']) . $required . '</label> <input data-meta="' . esc_attr($atts['meta_key']) . '" type="email" ' . $required_data . ' value="' . $value . '" ' . $name . ' ' . $placeholder . ' ' . $class . ' ' . $id . '>';
          break;
        case 'date':
        ...
        default:
          ...
          $field = '<label>' . esc_attr($atts['label']) . $required . '</label> <input data-meta="' . esc_attr($atts['meta_key']) . '" type="text" ' . $required_data . ' value="' . $value . '" ' . $name . ' ' . $placeholder . ' ' . $class . ' ' . $id . '>';
          break;
      }
      return $field;
    }
    ...
    $bookings = '
		<div class="eventer-dt-headers eventer-section-bookings">
			...
			<ul class="eventer-multi-nav">
				<li class="active eventer-get-user-bookings" data-book=""><a>' . esc_html__('All', 'eventer') . '</a></li>
				<li class=" eventer-get-user-bookings" data-book="passed"><a>' . esc_html__('Upcoming', 'eventer') . '</a></li>
				<li class=" eventer-get-user-bookings" data-book="upcoming"><a>' . esc_html__('Passed', 'eventer') . '</a></li>
			</ul>
		</div>';
    return $bookings;
  }
