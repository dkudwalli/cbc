  ...
  if ($original_event != $post->ID && $original_event != '') {
    ...
    <div class="eventer-form-table">
      ...
      <div class="eventer-metabox-section">
        <label><?php esc_html_e('Disable the bookings number of days before the event day Ex: 2. Leave blank to keep the bookings open until the event date/time.'); ?></label>
...
function wceventer_update_event_tickets_data($post_id, $post_object)
{
  ...
  if (isset($_POST['wceventer_ticket_name'])) {
    ...
    $allDates = get_post_meta($post_id, 'eventer_all_dates', true);
    $allDates = (array) $allDates;
    if (count($allDates) <= 1 || (isset($_POST['eventer_ticket_optional_settings']) && $_POST['eventer_ticket_optional_settings'] == 'all')) {
      global $wpdb;
